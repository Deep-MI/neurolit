...
def inpaint_volume(
    ...
    if SAVE_VOLUMES:
        nib.save(
            nib.Nifti1Image(val_image_inpainted[volume_only_slice].cpu().numpy() * 255, *affine_header),
...
def main(argv=None):
    ...
    if len(model_dict) == 0:
        ...
        sys.exit(f"ERROR: One or three checkpoints must be specified, but got {len(model_dict)}", file=sys.stderr)
